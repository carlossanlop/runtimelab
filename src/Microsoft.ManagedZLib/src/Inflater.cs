...
internal sealed class Inflater : IDisposable
{
    ...
    public bool Inflate(out byte b)
    {
        ...
        int bytesRead = Inflate(array.AsSpan());
    ...
    public int Inflate(byte[] bytes, int offset, int length)
    {
        ...
        return Inflate(bytes.AsSpan(offset, length));
    }
    ...
    public int Inflate(Span<byte> destination)
    {
        ...
        try
        {
            ...
            {
                ReadOutput(destination, out bytesRead);
            }
            else
            {
                if (_uncompressedSize > _currentInflatedCount)
                {
                    int length = (int)Math.Min(destination.Length, _uncompressedSize - _currentInflatedCount);
                    ReadOutput(destination.Slice(length), out bytesRead);
    ...
    private void ReadOutput(Span<byte> destination, out int bytesRead)
    {
        if (ReadInflateOutput(destination, ZLibFlushCode.NoFlush, out bytesRead) == ZLibErrorCode.StreamEnd)
        {
    ...
    private void InflateInit(int windowBits)
    {
        ...
        switch (error)
        {
            case ZLibErrorCode.Ok:           // Successful initialization
                return;

            case ZLibErrorCode.MemError:     // Not enough memory
                throw new ZLibException("SR.ZLibErrorNotEnoughMemory", "inflateInit2_", (int)error, _zlibStream.GetErrorMessage());

            case ZLibErrorCode.VersionError: //zlib library is incompatible with the version assumed
                throw new ZLibException("SR.ZLibErrorVersionMismatch", "inflateInit2_", (int)error, _zlibStream.GetErrorMessage());

            case ZLibErrorCode.StreamError:  // Parameters are invalid
    ...
    /// </summary>
    private ZLibErrorCode ReadInflateOutput(Span<byte> destination, ZLibFlushCode flushCode, out int bytesRead)
    {
        lock (SyncLock)
        {
            ...
            //ZLibErrorCode errC = Inflate(flushCode);
    ...
    /// </summary>
    private ZLibErrorCode Inflate(ZLibFlushCode flushCode)
    {
        ZLibErrorCode errC;
        try
        {
        ...
        switch (errC)
        {
            case ZLibErrorCode.Ok:           // progress has been made inflating
            case ZLibErrorCode.StreamEnd:    // The end of the input stream has been reached
                return errC;

            case ZLibErrorCode.BufError:     // No room in the output buffer - inflate() can be called again with more space to continue
                return errC;

            case ZLibErrorCode.MemError:     // Not enough memory to complete the operation
                throw new ZLibException("SR.ZLibErrorNotEnoughMemory", "inflate_", (int)errC, _zlibStream.GetErrorMessage());

            case ZLibErrorCode.DataError:    // The input data was corrupted (input stream not conforming to the zlib format or incorrect check value)
                throw new InvalidDataException("SR.UnsupportedCompression");

            case ZLibErrorCode.StreamError:  //the stream structure was inconsistent (for example if next_in or next_out was NULL),
    ...
    private bool IsInputBufferHandleAllocated => false; // _inputBufferHandle.Pointer != default;
}
